...
"""Generate edge-coherence example images for the 2D sensor documentation.

The examples isolate three ways a clean vertical edge can lose orientation
coherence: adding a stronger perpendicular line, thickening that perpendicular
line, and moving the perpendicular line toward the center-weighted detector
support. Detector scores are computed from raw patches first. The saved PNGs and
contact sheet then receive a visual arrow overlay showing the detected edge angle
so the annotation does not affect the measurement.

Usage from the project root:

    PYTHONPATH=/Users/hlee/tbp/feat.2d_sensor/src \
    /Users/hlee/miniconda3/bin/conda run -n tbp.monty \
        python sandbox/generate_edge_coherence_doc_images.py
"""
...
VERTICAL_LINE_RGB = (255, 255, 255)
ARROW_RGB = (255, 214, 0)
DEFAULT_OUTPUT_DIR = Path("write_ups/figures/edge_coherence_examples")


@dataclass(frozen=True)
class CoherenceCase:
    """Parameters for one edge-coherence documentation patch."""
    ...
    horizontal_y: int = IMAGE_SIZE // 2


@dataclass(frozen=True)
class ScoredCoherenceCase:
...
CASES = (
    ...
    CoherenceCase(
        row="horizontal_thickness",
        name="thickness_medium",
        label="thickness: 8",
        horizontal_gray=255,
        horizontal_thickness=8,
    ),
    CoherenceCase(
        row="horizontal_thickness",
        ...
        label="thickness: 24",
        horizontal_gray=255,
        horizontal_thickness=24,
    ),
    CoherenceCase(
        row="horizontal_offset",
        name="offset_far",
        label="offset: y 16",
        horizontal_gray=255,
        horizontal_y=16,
...
def render_raw_patch(case: CoherenceCase) -> np.ndarray:
    """Render an unannotated 64x64 RGB patch.
    ...
    cv2.line(
        ...
        (IMAGE_SIZE // 2, IMAGE_SIZE + IMAGE_SIZE // 2),
        VERTICAL_LINE_RGB,
        2,
        lineType=cv2.LINE_AA,
    )

    if case.horizontal_gray > 0 and case.horizontal_thickness > 0:
        horizontal_rgb = (
            case.horizontal_gray,
            case.horizontal_gray,
            case.horizontal_gray,
        )
        cv2.line(
            image,
            (-IMAGE_SIZE // 2, case.horizontal_y),
            (IMAGE_SIZE + IMAGE_SIZE // 2, case.horizontal_y),
            horizontal_rgb,
            case.horizontal_thickness,
            lineType=cv2.LINE_AA,
        )
...
def arrow_center(case: CoherenceCase, angle_rad: float) -> np.ndarray:
    """Choose an annotation center on the dominant line.

    Returns:
        Pixel coordinate where the display-only arrow should be centered.
    """
    angle_deg = np.degrees(angle_rad) % 180.0
    if angle_deg < 45.0 or angle_deg > 135.0:
        return np.array([IMAGE_SIZE / 2.0, float(case.horizontal_y)])
    return np.array([IMAGE_SIZE / 2.0, IMAGE_SIZE / 2.0])
...
def annotate_detected_angle(
    ...
    center = arrow_center(case, angle_rad)
    length = 16.0
...
def generate_examples(output_dir: Path) -> list[ScoredCoherenceCase]:
    """Generate all patches, annotated PNGs, and detector scores.

    Returns:
        Scored edge-coherence examples with raw and annotated images.
    """
    ...
    for index, case in enumerate(CASES, start=1):
        ...
        annotated_image = annotate_detected_angle(raw_image, case, edge.angle)
...
def write_manifest(path: Path, examples: list[ScoredCoherenceCase]) -> None:
            ...
            writer.writerow(
                {
                    ...
                    "horizontal_gray": example.case.horizontal_gray,
                    "horizontal_thickness": example.case.horizontal_thickness,
                    "horizontal_y": example.case.horizontal_y,
...
def make_contact_sheet(examples: list[ScoredCoherenceCase]) -> np.ndarray:
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.82\textwidth]{../figures/edge_coherence_examples/edge_coherence_contact_sheet.png}
  \caption{Synthetic 64x64 texture patches illustrating how \texttt{EdgeDetector}
  coherence decreases when a clean vertical line competes with perpendicular
  gradient evidence. Yellow arrows are drawn after scoring to show the detected
  edge angle without affecting the reported values.}
  \label{fig:edge-coherence-examples}
\end{figure}
""".strip()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate edge-coherence example images for 2D sensor docs."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        ...
        help=(
            ...
            "(default: write_ups/figures/edge_coherence_examples)."
        ),
    )
    parser.add_argument(
        "--no-latex-snippet",
...
def main(
    ...
    write_manifest(output_dir / "edge_coherence_examples.csv", examples)
    save_rgb_png(
        output_dir / "edge_coherence_contact_sheet.png",
        make_contact_sheet(examples),
    )

    for example in examples:
        print(
            f"{example.filename}: coherence={example.edge.coherence:.3f}, "
            f"strength={example.edge.strength:.3f}, "
            f"angle={format_angle_deg(example.edge.angle) or 'None'}, "
            f"has_edge={example.edge.has_edge}"
        )

    print(f"\nWrote edge-coherence documentation assets to {output_dir.resolve()}")
