"""Generate edge-within-edge diagonal pattern images.

This script creates three deterministic PNG images that show short local
diagonal strokes arranged along a larger global diagonal:

- diagonal_lines_64x64.png
- diagonal_lines_32x32.png
- diagonal_lines_16x16.png

By default, images are written to ``data/edge_within_edge``.

Usage from the project root:

    conda activate tbp.monty
    export PYTHONPATH="$(pwd)/src"
    python sandbox/generate_edge_within_edge_data.py

    conda activate tbp.monty
    export PYTHONPATH="$(pwd)/src"
    python sandbox/generate_edge_within_edge_data.py \
        --output-dir /tmp/edge_within_edge
"""
...
DEFAULT_OUTPUT_DIR = Path("data/edge_within_edge")
RESOLUTIONS = (
    (64, 1.0),
    (32, 0.5),
    (16, 0.25),
...
def generate_diagonal_pattern(
    ...
    segment_angle_deg: float = SEGMENT_ANGLE_DEG,
    ...
    Args:
        ...
        line_spacing: Spacing between stroke centers along the global diagonal
            (if None, auto-calculated)
        num_lines: Number of strokes to draw (if None, auto-calculated)
        global_angle_deg: Direction of the global diagonal
            (default: 45 = TL->BR in image coords)
        segment_angle_deg: Direction of each short stroke
            (default: -45 = BL->TR in image coords)
    ...
    image_size_hi = size * supersample

    # Create background at supersampled resolution
    image_hi = np.full((image_size_hi, image_size_hi, 3), bg_color, dtype=np.uint8)

    # Scale drawing parameters to supersampled resolution
    thickness_hi = max(1, round(line_thickness * supersample))
    margin_hi = round(size * margin_ratio * supersample)

    if line_length is None:
        # Keep strokes compact so adjacent strokes do not merge after downsampling.
    ...
    center_hi = np.array([image_size_hi / 2.0, image_size_hi / 2.0], dtype=np.float32)
    ...
    # Evaluate candidates and take min/max valid t where point remains inside
    # on both axes.
    ts_valid = []
...
def main(output_dir: Path = DEFAULT_OUTPUT_DIR) -> list[Path]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Generate reproducible edge-within-edge diagonal pattern PNGs at "
            "64x64, 32x32, and 16x16."
        ),
        epilog=(
            "Examples:\n"
            "  python sandbox/generate_edge_within_edge_data.py\n"
