...
The 2D movement rewrite has its own state machine.
If the observation is off object, the method emits zero displacement and leaves the previous on-object 3D and 2D locations unchanged.
If this is the first on-object observation, it seeds \texttt{\_previous\_3d\_location} from the real 3D location and seeds \texttt{\_previous\_2d\_location} from the current world \(x, y\), then emits \([x, y, 0]\).
On later on-object observations, it computes the 3D displacement from the previous on-object point, projects that displacement onto the tangent plane of \texttt{true\_surface\_normal}, and expresses the tangent component in the current transported basis:
\[
  du = d_{\mathrm{tan}} \cdot u,
  \qquad
  dv = d_{\mathrm{tan}} \cdot v
\]
If curvature magnitudes and the saved curvature pose are available, \((du, dv)\) are passed through \texttt{arc\_length\_corrected\_displacement}; otherwise the projected chord-length components are accumulated as-is.
The final \((du, dv)\) is added to the private 2D location, emitted as \([du, dv, 0]\), and the accumulated location is emitted as \([x_{2d}, y_{2d}, 0]\).

Several details follow from this ordering.
The tangent frame is updated before edge extraction so the image-space edge angle can be projected into the current local 2D basis.
The same transported basis is then reused for the movement update, so edge pose and displacement are expressed in the same local chart during the step.
The 2D position update also runs after motor-only handling; a motor-only message can update the private trajectory even though \texttt{use\_state=False} prevents the message from being consumed as a percept.
...
The first two location coordinates also change meaning over time: the first on-object location is seeded from world \(x, y\), while later locations are accumulated tangent-frame displacements added to that seed.
This works as an episode-local surface chart, but it does not make traces automatically comparable across different starting points or different initial tangent-frame orientations.
...
There is one implementation limitation to keep in mind when reading the current code.
The 2D position update looks for \texttt{principal\_curvatures} in the morphological feature dictionary, while the standard observation processor stores that value as a non-morphological feature.
The saved curvature pose is still necessary and correct, but the arc-correction branch is skipped in the default runtime path unless the curvature magnitudes are supplied where the update currently looks for them.
Resolving that mismatch should be treated as a separate implementation fix rather than folded into the documentation contract.
