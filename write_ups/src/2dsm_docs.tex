...
A limitation of \texttt{EdgeDetector} is that it returns one dominant orientation per patch. If the patch contains more than one edge, then the result may output an orientation that is some 

% Comment: Say that we use Zoom = 30 to increase our chance of having one edge per patch

An enhancement to \texttt{EdgeDetector} would be to 
...
% Comment: I think we should state briefly how to overcome the previous sentence. 
...
% Comment:
% 1. Limitation: TwoDSensorModule can only be trained on single rotation (0, 0, 0)
% 2. Limitation: Whether we can learn a perfectly unrolled surface depends on the Gaussian curvature (developable surface), things like sphere will introduce distortions, or have different learned models depending on the path Monty/agent takes to explore, but this hasn't been thoroughly studied. 
% 3. Limitation: TwoDSensorModule hasn't been trained with PerceptFilter, i.e. every observation is sent to LM. It may be problematic to send with PerceptFilter because if we don't send information for a while, then our overall step size or 3D displacement may be large, and our estimatino for 2D displacement may not be as accurate. 
% 4. The main thing is: we must take small steps. 
